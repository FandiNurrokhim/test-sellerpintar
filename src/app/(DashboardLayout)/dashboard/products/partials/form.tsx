...
import { CategoryFormData } from "@/schemas/products/categories";
...
export const ProductModal: React.FC<ProductModalProps> = ({
  ...
  const handleCreateCategory = async (categoryName: string): Promise<void> => {
    ...
    try {
      ...
      if (newCategory?.data?.id) {
        selectedCategoryId = newCategory.data.id;
      } else if (newCategory?.data?._id) {
        selectedCategoryId = newCategory.data._id;
      }

      if (!selectedCategoryId) {
        const updatedCategories = (await productApi.getCategories(
          true,
          {},
          headers
        )) as CategoryApiResponse;
        ...
        if (
          ...
          Array.isArray(updatedCategories.data.categories)
        ) {
          const createdCategory = updatedCategories.data.categories.find(
            (cat: Category) => cat.name === categoryName
          );

          if (createdCategory) {
            selectedCategoryId =
              createdCategory.id || createdCategory._id || null;
          }
        }
      }
      ...
      showToast("Failed to add category: " + errorMessage, "error");
    } finally {
  ...
  const onFormSubmit = async (data: ProductFormData): Promise<void> => {
    try {
      ...
      await productApi.create(cleanData, true, {}, headers);
      ...
      <div className="flex flex-col h-full">
        ...
        >
          <div className="space-y-4">
            ...
            <FormField
              label={
                <Label className="!font-sentient" htmlFor="name" required>
                  Product Name
                </Label>
              }
              input={
            ...
            <div className="grid grid-cols-2 gap-4">
              <FormField
                label={
                  <Label className="!font-sentient" htmlFor="price" required>
                    Price
                  </Label>
                }
                input={
              ...
              <FormField
                label={
                  <Label
                    ...
                    htmlFor="stock"
                    required
                    tooltip="Stock is the total quantity available for this product. It will be automatically updated based on variant stock if variants are used."
                  >
                    Stock
                  </Label>
                }
